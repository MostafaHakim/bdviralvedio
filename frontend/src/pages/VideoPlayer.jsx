...
const VideoPlayer = () => {
  ...
  const handleComment = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/videos/${id}/comment`, {
        method: "POST",
        headers: {
          ...
          <div className="flex items-center space-x-4">
            {user ? (
              <div className="flex items-center space-x-3">
                ...
                <img src={user.image || "https://ui-avatars.com/api/?name=" + user.name} className="w-9 h-9 rounded-full object-cover" alt="profile" />
              </div>
            ) : (
              <Link to="/login" className="text-sm font-medium text-red-600">Login</Link>
            )}
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1400px] mx-auto p-4 lg:p-6 flex flex-col lg:flex-row gap-8">
        <div className="flex-1">
          <div className="aspect-video w-full rounded-xl overflow-hidden bg-black shadow-lg">
            {video.url.includes("youtube.com") || video.url.includes("youtu.be") ? (
              <iframe className="w-full h-full" src={video.url.includes("embed") ? video.url : video.url.replace("watch?v=", "embed/")} title={video.title} allowFullScreen></iframe>
            ) : (
              <video className="w-full h-full" src={video.url} controls autoPlay></video>
            )}
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-4 border-b pb-5">
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 rounded-full bg-red-600 flex items-center justify-center text-white font-bold">B</div>
                <div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <button className="bg-gray-100 px-5 py-2 rounded-full text-sm font-medium">Share</button>
              </div>
            </div>
            ...
            <div className="mt-8">
              ...
              {user ? (
                <form onSubmit={handleComment} className="flex space-x-4 mb-8">
                  <img src={user.image || "https://ui-avatars.com/api/?name=" + user.name} className="w-10 h-10 rounded-full object-cover" alt="" />
                  <div className="flex-1">
                    <input 
                      type="text" 
                      value={newComment} 
                    ...
                    <div className="flex justify-end mt-2">
                      <button type="submit" className="bg-red-600 text-white px-4 py-2 rounded-full text-sm font-bold">Comment</button>
                    </div>
                  </div>
                </form>
              ) : (
                <div className="bg-gray-100 p-4 rounded-xl text-center mb-8">
                  <p className="text-sm text-gray-600">Please <Link to="/login" className="text-red-600 font-bold">Login</Link> to post a comment</p>
                </div>
              )}
              ...
              <div className="space-y-6">
                {comments.map((comment) => (
                  <div key={comment._id} className="flex space-x-4">
                    <img src={comment.user.image || "https://ui-avatars.com/api/?name=" + comment.user.name} className="w-10 h-10 rounded-full object-cover" alt="" />
                    <div>
        ...
        <div className="lg:w-[400px] space-y-4">
          ...
          {relatedVideos.map((rv) => (
            <Link key={rv._id} to={`/video/${rv._id}`} className="flex space-x-3 group">
              <img src={rv.thumbnail} className="w-40 h-24 object-cover rounded-xl shadow-sm" alt="" />
              <div className="flex-1">
                <h3 className="text-sm font-bold line-clamp-2 group-hover:text-red-600 transition">{rv.title}</h3>
